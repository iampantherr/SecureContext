...
assert("Fresh project: limit = 50 (base)",  p1.computedLimit === 50,  `got ${p1.computedLimit}`);
assert("Fresh project: evictTo = 40 (80%)", p1.evictTo === 40,        `got ${p1.evictTo}`);
console.log(`  [INFO] Formula: 50 + kb(0) + bc(0) + agents(0) = ${p1.computedLimit}, evictTo=${p1.evictTo}`);
...
assert("Force recompute: same limit on fresh DB",  limits1b.max === 50);
...
assert("KB-heavy: limit > 50",               p2.computedLimit > 50,      `got ${p2.computedLimit}`);
assert("KB-heavy: limit = 70",               p2.computedLimit === 70,    `got ${p2.computedLimit}`);
assert("KB-heavy: evictTo = 56 (80%)",       p2.evictTo === 56,          `got ${p2.evictTo}`);
console.log(`  [INFO] Formula: 50 + kb(${Math.min(Math.floor(300/15),60)}) + bc(0) + agents(0) = ${p2.computedLimit}, evictTo=${p2.evictTo}`);
...
assert("BC-heavy: limit = 56",              p3.computedLimit === 56,   `got ${p3.computedLimit}`);
assert("BC-heavy: evictTo = 44 (80%)",      p3.evictTo === 44,         `got ${p3.evictTo}`);
console.log(`  [INFO] Formula: 50 + kb(0) + bc(${Math.min(Math.floor(180/30),40)}) + agents(0) = ${p3.computedLimit}, evictTo=${p3.evictTo}`);
...
assert("Multi-agent: limit = 100",          p4.computedLimit === 100, `got ${p4.computedLimit}`);
assert("Multi-agent: evictTo = 80 (80%)",   p4.evictTo === 80,        `got ${p4.evictTo}`);
console.log(`  [INFO] Formula: 50 + kb(0) + bc(0) + agents(${Math.min(4*15,50)}) = ${p4.computedLimit}, evictTo=${p4.evictTo}`);
...
assert("Full-platform: limit = 130",         p5.computedLimit === 130, `got ${p5.computedLimit}`);
assert("Full-platform: evictTo = 104",       p5.evictTo === 104,       `got ${p5.evictTo}`);
console.log(`  [INFO] Formula: 50 + kb(20) + bc(10) + agents(50) = ${p5.computedLimit}, evictTo=${p5.evictTo}`);

// ─── Test 7: Cap enforcement ──────────────────────────────────────────────────
console.log("\n--- Test 7: Cap enforcement (900+ KB, 1200+ BC, 5 agents → max 200) ---");
...
function simulateComplexity(kbEntries, broadcastCount, activeAgents) {
  ...
  return Math.max(50, Math.min(200, 50 + kbBonus + bcBonus + agentBonus));
}

assert("Max cap: 900 KB + 1200 BC + 5 agents = 200",
  simulateComplexity(900, 1200, 5) === 200,
  `got ${simulateComplexity(900, 1200, 5)}`);

assert("Mid cap: 450 KB + 600 BC + 3 agents = 50+30+20+45=145",
  simulateComplexity(450, 600, 3) === 145,
  `got ${simulateComplexity(450, 600, 3)}`);

assert("Floor hold: 5 KB + 2 BC + 0 agents = 50 (base)",
  simulateComplexity(5, 2, 0) === 50,
  `got ${simulateComplexity(5, 2, 0)}`);

assert("Single solo dev: 20 KB + 10 BC + 1 agent = 50+1+0+15=66",
  simulateComplexity(20, 10, 1) === 66,
